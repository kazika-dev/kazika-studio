...
ElevenLabs ノードでテキストフィールドにタグを挿入する機能を実装しました。`kazikastudio.eleven_labs_tags` テーブルに登録されたタグを選択し、`[タグ名]` 形式でテキストフィールドに挿入できます。
...
- タグ一覧を表示し、クリックで対象フィールドにタグを挿入する UI を実装
...
2. 設定パネルで「テキスト」フィールドにカーソルを置く
3. 「タグ挿入」セクションからタグをクリック
4. タグが `[タグ名]` 形式でテキストフィールドに挿入される

### /form ページでの使用

1. ElevenLabs ノードを含むワークフローの `/form` ページにアクセス
2. テキストフィールドの下に「タグ挿入」ボタンが表示される
3. タグをクリックするとテキストフィールドにタグが挿入される
