...
CLAUDE.mdは40.0k文字以下にして概要としてまとめてください。
追加修正した詳細は `/docs` ディレクトリにまとめてCLAUDE.mdからリンクしてください。
...
- **[プロンプトキュー機能](/docs/prompt-queue.md)** - 画像生成用のプロンプトと参照画像をキューとして保存・管理する機能

## 最近の主要な変更

### 2025-12-23: プロンプトキューのステータス編集機能を追加

**目的**: プロンプトキューの編集ダイアログでキューのステータス（待機中、処理中、完了、失敗、キャンセル）を変更できるようにする

**変更内容**:

1. **PromptQueueDialog の修正** (`/components/prompt-queue/PromptQueueDialog.tsx`)
   - `PromptQueueStatus` 型をインポート
   - `STATUS_OPTIONS` 定数を追加（各ステータスにラベルと色を定義）
   - `status` 状態変数を追加
   - 編集時にステータスを初期化し、保存時に送信
   - ステータス選択用のドロップダウンUIを追加（編集時のみ表示）
   - 各ステータスに色付きの丸印アイコンを表示
...
- `completed` - 完了（緑）
- `failed` - 失敗（赤）
- `cancelled` - キャンセル（オレンジ）

**技術的詳細**:
- 新規作成時はステータスフィールドを表示しない（デフォルトで `pending`）
- 編集時のみステータス変更が可能
- `PUT /api/prompt-queue/[id]` エンドポイントは既に `status` の更新に対応済み

**影響範囲**:
- プロンプトキューの編集ダイアログでステータスを直接変更可能に
- 例：失敗したキューを手動で `pending` に戻して再実行する運用が可能に
...
1. **「実行時のAPIリクエスト」セクションを削除**
   - WorkflowStepCardから重複セクションを削除（525-575行）
   - この情報はNodeExecutionCard内に統合されているため不要

2. **「ノード実行結果」→「ノード」に改名**
   - セクションタイトルを変更し、より包括的な表現に

3. **NodeExecutionCardに3つのセクションを表示**
   - **ワークフロー入力（読み取り専用）**: ステップ全体の入力からこのノードに関連する項目をフィルタリング
   - **ノード入力（編集可能）**: `metadata.execution_requests[nodeId]` の内容を表示・編集
   - **実行結果**: `output_data[nodeId]` の出力データを表示

4. **ワークフロー入力のフィルタリング機能**
   - `getRelatedWorkflowInputs()` 関数で、ノードIDを含むキーのみを抽出
   - 例: `gemini_prompt_node-123` → ノード `node-123` の `prompt` フィールド
   - 青い背景で読み取り専用であることを視覚的に表示

5. **Next.js 15 対応**
   - `params` を `Promise` として扱うように修正
   - `/api/studios/steps/[id]/execute-node` と `/api/studios/steps/[id]/update-node-inputs` で対応

**データフロー**:
```
WorkflowStepCard
  ├─ ワークフロー入力（ステップ全体、読み取り専用）
  │   └─ step.input_config.workflowInputs
  │
  └─ ノード（各ノード個別）
      ├─ ワークフロー入力（読み取り専用）
      │   └─ workflowInputs から当該ノードに関連する項目をフィルタ
      ├─ ノード入力（編集可能）
      │   └─ metadata.execution_requests[nodeId]
      └─ 実行結果
          └─ output_data[nodeId]
```

**技術的詳細**:
- **フィルタリングロジック**: `key.includes(\`_${node.id}\`)` でノードIDを含むキーのみを抽出
- **視覚的区別**:
  - ワークフロー入力: 青色背景（`info.50`）、読み取り専用
  - ノード入力: グレー背景（`grey.50`）、編集ボタン付き
  - 実行結果: 白背景、OutputDataDisplayで適切に表示

**影響範囲**:
- ✅ 情報の重複を完全に排除
- ✅ 各ノードで必要な情報がすべて1箇所で確認可能
- ✅ ワークフロー入力とノード入力の区別が明確
- ✅ Next.js 15の非同期paramsに対応してエラーを解消
