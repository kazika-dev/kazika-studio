...
-- =====================================================
-- 1. prompt_queue テーブル（メインテーブル）
-- =====================================================
CREATE TABLE IF NOT EXISTS kazikastudio.prompt_queue (
...
-- ステータス値のチェック制約
ALTER TABLE kazikastudio.prompt_queue
  ADD CONSTRAINT chk_prompt_queue_status
  CHECK (status IN ('pending', 'processing', 'completed', 'failed', 'cancelled'));

-- インデックス
CREATE INDEX IF NOT EXISTS idx_prompt_queue_user_id ON kazikastudio.prompt_queue(user_id);
CREATE INDEX IF NOT EXISTS idx_prompt_queue_status ON kazikastudio.prompt_queue(status);
CREATE INDEX IF NOT EXISTS idx_prompt_queue_priority_created ON kazikastudio.prompt_queue(priority DESC, created_at ASC);

-- RLS有効化
ALTER TABLE kazikastudio.prompt_queue ENABLE ROW LEVEL SECURITY;

-- RLSポリシー: ユーザーは自分のキューのみアクセス可能
DROP POLICY IF EXISTS "Users can view own prompt_queue" ON kazikastudio.prompt_queue;
CREATE POLICY "Users can view own prompt_queue"
  ON kazikastudio.prompt_queue
  FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own prompt_queue" ON kazikastudio.prompt_queue;
CREATE POLICY "Users can insert own prompt_queue"
  ON kazikastudio.prompt_queue
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own prompt_queue" ON kazikastudio.prompt_queue;
CREATE POLICY "Users can update own prompt_queue"
  ON kazikastudio.prompt_queue
  FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own prompt_queue" ON kazikastudio.prompt_queue;
CREATE POLICY "Users can delete own prompt_queue"
  ON kazikastudio.prompt_queue
  FOR DELETE
  USING (auth.uid() = user_id);

-- updated_at自動更新用トリガー
CREATE OR REPLACE FUNCTION kazikastudio.update_prompt_queue_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS update_prompt_queue_updated_at ON kazikastudio.prompt_queue;
CREATE TRIGGER update_prompt_queue_updated_at
  BEFORE UPDATE ON kazikastudio.prompt_queue
  FOR EACH ROW
  EXECUTE FUNCTION kazikastudio.update_prompt_queue_updated_at();

-- パーミッション付与
GRANT ALL ON kazikastudio.prompt_queue TO anon, authenticated;
GRANT USAGE, SELECT ON SEQUENCE kazikastudio.prompt_queue_id_seq TO anon, authenticated;
...
CREATE TABLE IF NOT EXISTS kazikastudio.prompt_queue_images (
  id BIGSERIAL PRIMARY KEY,
  queue_id BIGINT NOT NULL REFERENCES kazikastudio.prompt_queue(id) ON DELETE CASCADE,
...
CREATE POLICY "Users can view own prompt_queue_images"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM kazikastudio.prompt_queue
      WHERE prompt_queue.id = prompt_queue_images.queue_id
      AND prompt_queue.user_id = auth.uid()
    )
  );
...
-- コメント
-- =====================================================
COMMENT ON TABLE kazikastudio.prompt_queue IS 'プロンプトキュー - 画像生成用のプロンプトと参照画像を保存';
COMMENT ON COLUMN kazikastudio.prompt_queue.status IS 'ステータス: pending, processing, completed, failed, cancelled';
COMMENT ON COLUMN kazikastudio.prompt_queue.priority IS '優先度 - 高いほど先に処理される';
COMMENT ON COLUMN kazikastudio.prompt_queue.output_id IS '生成結果のworkflow_outputs.id';
